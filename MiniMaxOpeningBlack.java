...
public class MiniMaxOpeningBlack extends Morris {

	static int totalpositionevaluated;
	static int score = Integer.MIN_VALUE;

	public static List<Character> minimaxopeningblack(List<Character> board, int depth) {
		...
		allPossibleMoves = Morris.GenerateMovesOpening(board);
		...
		for (int j = 0; j < successmove.size(); j++) {
			if (successmove.get(j) == 'W')
				successmove.set(j, 'B');
			else if (successmove.get(j) == 'B')
				successmove.set(j, 'W');
	...
	public static int minimax(List<Character> list1, int maxplayer, int depth) {
		int fscore = 0;
		int tempscore;
		...
		if (depth == 0) {
			...
			return Morris.stestopening(list2);

		}

		if (maxplayer == 1) {

			allPossMove = Morris.GenerateMovesOpening(list2);
